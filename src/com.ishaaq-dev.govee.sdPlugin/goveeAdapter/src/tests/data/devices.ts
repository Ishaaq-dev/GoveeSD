...
function getDevices() {
    const devices: { [key: string]: { name: string, address: string, sku: string, type: string } } = {};
...
export function getDevice(deviceName: DEVICE_NAMES) {
    switch (deviceName) {
        case DEVICE_NAMES.BOOKSHELF_LAMP:
            return {
        ...
        default:
            throw new Error(`Incorrect deviceName passed: ${deviceName}`);
    }
}
...
export function getDevicesResponse() {
    return {
        ...
        "message": "success",
        "data": [
            getDevice(DEVICE_NAMES.BOOKSHELF_LAMP),
            getDevice(DEVICE_NAMES.DESK_LIGHTS),
            getDevice(DEVICE_NAMES.HEATER)
        ]
    }
}